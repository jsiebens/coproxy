...
func (l *lookupLib) Lookup(service string) ([]string, error) {
	srvs, err := l.LookupSRV(service)
	if err != nil {
		return nil, err
	}
	...
	for _, srv := range srvs {
		a, err := l.LookupA(srv.Target)
		if err == nil {
			result = append(result, fmt.Sprintf("%s:%d", a, srv.Port))
		}
	}

...
RUN VERSION=$(git describe --all --exact-match `git rev-parse HEAD` | grep tags | sed 's/tags\///') \
    ...
    && GOOS=${TARGETOS} GOARCH=${TARGETARCH} CGO_ENABLED=${CGO_ENABLED} go build \
        --ldflags "-s -w \
        -X github.com/jsiebens/coproxy/version.GitCommit=${GIT_COMMIT}\
        -X github.com/jsiebens/coproxy/version.Version=${VERSION}" \
        -a -installsuffix cgo -o coproxy .
